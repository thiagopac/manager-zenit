    ...
    <div class="nano-content">
        <div class="header">
            <div class="message-content-menu">
                <button class="message-reply-button btn btn-success" <?=count($actions) > 0 && $travel_order->finished != 1 ? '' : 'disabled'; ?> role="button">
                    <i class="icon dripicons-forward"></i> <?=$this->lang->line('application_actions'); ?>
                </button>
<!--                    <a class="btn btn-danger" href="--><?//=base_url()?><!--travelorders/delete/--><?//=end($conversation)->id?><!--" role="button"><i class="icon dripicons-trash"></i> --><?//=$this->lang->line('application_delete'); ?><!--</a>-->
            </div>
            ...
            <h1 class="page-title"><a class="icon glyphicon glyphicon-chevron-right trigger-message-close"></a><br><span class="dot"></span><?=$value->subject; ?>
                <span class="grey">[<?=$this->lang->line('application_travel_order');?> <?=$travel_order->id?>] - <?=$travel_order->subject?></span>
            </h1>
            ...
            <?php $unix = human_to_unix($travel_order->created_at); ?>
            <p class="subtitle"><?=$this->lang->line('application_claimant'); ?>: <?=$travel_order->user->firstname?>, <?=$this->lang->line('application_started_on'); ?> <?php  echo date($core_settings->date_format . ' ' . $core_settings->date_time_format, $unix); ?></p>
        </div>
        ...
        <div class="message-container">
            ...
            <div class="container-fluid" style="width:100%;margin: 0px;padding: 0px;">
                <ul class="list-unstyled multi-steps" style="cursor: pointer">
                    <?php if ($travel_order->canceled != true) : ?>
                        <?php foreach ($steps as $step) : ?>
                            <li title="<?=implode('&#xD;', array_column($step->members, 'name'))?>" class="<?=$travel_order->step == $step->id && $travel_order->finished != 1 ? 'is-active' : ''; ?>"><?=$step->name; ?></li>
                        <?php endforeach; ?>
                    <?php else : ?>
            ...
            <div class="message-content-reply">
                ...
                echo form_open_multipart('travelorders/reply', $attributes); ?>
                ...
                <div class="container-fluid">
                    ...
                    <div class="textarea-footer message-footer">
                        ...
                        <?php foreach ($actions as $action) : ?>
                            ...
                            <?php endif; ?>
                        <?php endforeach; ?>
                        <a data-toggle="mainmodal" href="<?= base_url() ?>travelorders/timetravel/<?= $travel_order->id; ?>" class="pull-right btn btn-warning">Alterar destino da OV</a>
            ...
            <div>
                <h3><?=$this->lang->line('application_travel_order_history');?></h3>
                                    ...
                                    <?php foreach ($reg->history_data as $reg_data) :?>
                                        <p><b><?=implode(' ',(explode('_', $reg_data->label)));?>:</b>
                                            ...
                                            if ($reg_data->className == "form-control mask-money"){
                                                ...
                                                <a href="<?=base_url()?>files/travelorders/<?=$reg_data->value?>" target="_blank"><i class="dripicons dripicons-document" style="font-size: 22px"></i>
                                ...
                                <?php if ($reg->history_files != null) : ?>
                                    ...
                                    <p>
                                        <?php foreach ($reg->history_files as $file) : ?>
                                            ...
                                            <a href="<?=base_url()?>files/travelorders/<?=$file?>" target="_blank"><i class="dripicons dripicons-document" style="font-size: 22px"></i>
            ...
            <div>
                <h3><?=$this->lang->line('application_travel_order_data');?></h3>
                            ...
                            <?php if ($response->$fieldname != null) :  ?>
                                ...
                                <?php else : ?>
                                    ...
                                    <td><?php foreach ($response->$fieldname as $field) : ?>
                                            <a href="<?=base_url()?>files/travelorders/<?=$field?>" target="_blank"><i class="dripicons dripicons-document" style="font-size: 22px"></i>
