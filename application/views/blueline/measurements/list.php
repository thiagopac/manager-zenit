...
if($measurements){
    foreach ($measurements as $value):
        //var_dump(json_decode($value->flow)->steps[0]->id);exit;
        //var_dump(json_decode($value->flow)->steps[0]->members[0]->name);exit;
        $datetime = new DateTime($value->created_at);?>
        <li class="<?php if($filter){echo $filter;}?> hidden" data-link="<?=base_url()?>measurements/view/<?=$value->id;?><?php if(isset($filter)){echo "/".$filter;} ?><?php if(isset($filter)){ if($filter == "Sent"){echo "/".$value->id;} } ?>">
            <div class="col col-1">
                <p class="title">
            ...
            <div class="col col-2">
                <div class="subject"><?=$value->company;?></div>
                <div class="date">
...
<!--        <a id="prev" name="prev" class="message-list-load sent-folder btn btn-primary" href="--><?//=base_url()."measurements/listing/".$obj_list_page_prev?><!--">Anterior</a>-->
<!--        <a id="next" name="next" class="message-list-load sent-folder btn btn-primary" href="--><?//=base_url()."measurements/listing/".$obj_list_page_next?><!--">Próxima</a>-->
<!--    </div>-->
<?php //else : ?>
<!--    <div class="col-md-12" align="center">-->
<!--        <a id="prev" name="prev" class="message-list-load sent-folder btn btn-primary" href="--><?//=base_url()."measurements/filter/".strtolower($filter)."/".$obj_list_page_prev?><!--">Anterior</a>-->
<!--        <a id="next" name="next" class="message-list-load sent-folder btn btn-primary" href="--><?//=base_url()."measurements/filter/".strtolower($filter)."/".$obj_list_page_next?><!--">Próxima</a>-->
<!--    </div>-->
<?php //endif; ?>
...
<style>
    ...
    #measurementslist{
        /*bottom: */<?//=count($measurements) * 0.55 ?>/*mm !important;*/
