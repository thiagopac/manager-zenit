<div class="col-sm-13 col-md-12 main">
	<div class="row">
		...
		<div id="kanban-page">
                ...
                <a v-if="stages.length > 1" href="<?=base_url()?>terrains/create" class="btn btn-primary" data-toggle="mainmodal" v-cloak>
                    <?=$this->lang->line('application_create_terrain');?>
                </a>

            <?php if($condition == 1) { ?>

                <a href="<?=base_url()?>terrains/status/create" class="btn btn-primary" data-toggle="mainmodal" v-cloak>
			...
			<div class="col-md-4 select-wrapper pull-right" v-cloak>
                ...
                <a href="<?=base_url()?>terrains/tags" class="btn btn-success">
                    <?=$this->lang->line('application_edit_tags');?>
                </a>

                <a href="<?=base_url()?>terrains/lost" class="btn btn-danger">
                    <?=$this->lang->line('application_show_lost_terrains');?>
                </a>
            ...
            <div  id="block-all-terrains" style="background: #f1f4fa">

                <kanban-board :stages="stages" :blocks="getTerrains" @update-block="updateBlock" @delete-block="deleteBlock">
                        ...
                        <?php if($condition == 1) { ?>
                            ...
                            <div class="btn-group pull-right-responsive">
                                ...
                                <ul class="dropdown-menu pull-right" role="menu">
                                    <li>
                                        <a data-toggle="mainmodal" :href="'<?=base_url()?>terrains/status/edit/'+stage.id">
                    ...
                    <div v-for="(block, index) in getTerrains" :slot="block.id" v-cloak>

                        <div>
                            <!--						<i class="icon dripicons-lock" style="font-size: 20px; color: orangered;" title="--><?//=htmlspecialchars($this->lang->line('application_private_terrain'));?><!--"-->
                        ...
                        <div class="block-details" v-if="block.id == openBlock" :class="(block.id == openBlock) ? 'block-details-open' : ''">
                            <ul class="nav nav-tabs nav-tabs-terrain">
                                ...
                                <!--                            --><?php //if($condition == 1) { ?>
                                <li>
                                    <a data-toggle="mainmodal" :href="'<?=base_url()?>terrains/edit/'+block.id">
                            ...
                            <div class="tab-content">
                                ...
                                <div class="tab-pane tab2" v-if="openActivities == block.id" :class="(openActivities == block.id) ? 'in active animated slideInRight' : '' ">
                                    ...
                                    <div class="form-group filled chat_message_input">
										...
										<input type="file" name="userfile" @change="uploadAttachment" :data-image-holder="'image_holder_'+block.id" class="chat-attachment terrain-attachment hidden">

									</span>
                                        <div class="terrain-image-preview" v-if="attachment.file">
                                            <div class="terrain-upload-progress" v-if="uploadProcess">
                                                {{ uploadProcess }}%
                                                <div class="progress">
                                    ...
                                    <div v-if="!activitiesLoading">
                                        <transition-group name="list" tag="ul" class="terrain-comments" appear>
                                            <li v-for="comment in comments" v-bind:key="comment.id">
                                                ...
                                                <div class="terrain-attachment" v-if="comment.attachment">
                                                    <a v-if="isImage(comment.attachment_link)" :href="'<?=base_url()?>files/media/'+comment.attachment_link" :data-lightbox="'terrain'+comment.terrain_id"
                                                       :data-title="comment.attachment">
                                                        <img :src="'<?=base_url()?>files/media/thumb_'+comment.attachment_link" />
                                                    </a>
                                                    <a v-if="!isImage(comment.attachment_link)" :href="'<?=base_url()?>terrains/attachment/'+comment.id">
                                ...
                                <div class="tab-pane tab3" v-if="openReminders == block.id" :class="(openReminders == block.id) ? 'in active animated slideInRight' : '' ">
                                    ...
                                    <a :href="'<?=base_url()?>terrains/reminder/create/'+block.id" class="btn btn-primary btn-block terrain-reminder-button" data-toggle="mainmodal">
                                    ...
                                    <transition-group name="list" tag="ul" class="terrain-reminders" appear>
                                        <li v-for="(reminder, index) in reminders" v-bind:key="reminder.id">
                                            <div class="reminder-wrapper has-hover-options" :class="[(isDue(reminder.datetime) && reminder.done == 0) ? 'red' : 'green', (reminder.done == 1) ? 'gray' : '']">
                                                ...
                                                <div class="pull-right hover-options">
                                                    ...
                                                    <a :href="'<?=base_url()?>terrains/reminder/edit/'+reminder.id" data-toggle="mainmodal">
                        ...
                        <div class="row block-actions" :class="block.completed">
                            ...
                            <!--<a class="col-xs-2 center" :class="(block.private != '1') ? '' : ''" title="Histórico do Terrain">
...
jQuery(document).ready(function($) {
    ...
    $(document).bind("fullscreenchange", function() {
        if (!$(document).fullScreen()){
            $(".drag-column").removeClass("terrain-screen-tv-size");
            $(".block-title").removeClass("terrain-title-tv-size");
    ...
    $(".tv-screen").on('click', function () {
        if ($("#block-all-terrains").fullScreen() != null) {
            $(".drag-column").addClass("terrain-screen-tv-size");
            $(".block-title").addClass("terrain-title-tv-size");
            $(".tippy").addClass("terrain-status-tv-title");
            $("#block-price, #block-subtitle").toggle();
...
<?php if (isset($search)) :?>
	<script>
        sessionStorage . setItem('terrain', '<?=$search?>');
