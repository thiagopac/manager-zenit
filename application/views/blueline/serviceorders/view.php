    ...
    <div class="nano-content">
        <div class="header">
            <div class="message-content-menu">
                <button class="message-reply-button btn btn-success" <?=count($actions) > 0 && $service_order->finished != 1 ? '' : 'disabled'; ?> role="button">
                    <i class="icon dripicons-forward"></i> <?=$this->lang->line('application_actions'); ?>
                </button>
<!--                    <a class="btn btn-danger" href="--><?//=base_url()?><!--serviceorders/delete/--><?//=end($conversation)->id?><!--" role="button"><i class="icon dripicons-trash"></i> --><?//=$this->lang->line('application_delete'); ?><!--</a>-->
            </div>
            ...
            <h1 class="page-title"><a class="icon glyphicon glyphicon-chevron-right trigger-message-close"></a><br><span class="dot"></span><?=$value->subject; ?>
                <span class="grey">[<?=$this->lang->line('application_service_order');?> <?=$service_order->id?>] - <?=$service_order->subject?></span>
            </h1>
            ...
            <?php $unix = human_to_unix($service_order->created_at); ?>
            <p class="subtitle"><?=$this->lang->line('application_claimant'); ?>: <?=$service_order->user->firstname?>, <?=$this->lang->line('application_started_on'); ?> <?php  echo date($core_settings->date_format . ' ' . $core_settings->date_time_format, $unix); ?></p>
        </div>
        ...
        <div class="message-container">
            ...
            <div class="container-fluid" style="width:100%;margin: 0px;padding: 0px;">
                <ul class="list-unstyled multi-steps" style="cursor: pointer">
                    <?php if ($service_order->canceled != true) : ?>
                        <?php foreach ($steps as $step) : ?>
                            <li title="<?=implode('&#xD;', array_column($step->members, 'name'))?>" class="<?=$service_order->step == $step->id && $service_order->finished != 1 ? 'is-active' : ''; ?>"><?=$step->name; ?></li>
                        <?php endforeach; ?>
                    <?php else : ?>
            ...
            <div class="message-content-reply">
                ...
                echo form_open_multipart('serviceorders/reply', $attributes); ?>
            ...
            <div>
                <h3><?=$this->lang->line('application_service_order_history');?></h3>
                                ...
                                <?php if ($reg->history_files != null) : ?>
                                    ...
                                    <p>
                                        <?php foreach ($reg->history_files as $file) : ?>
                                            ...
                                            <a href="<?=base_url()?>files/serviceorders/<?=$file?>" target="_blank"><i class="dripicons dripicons-document" style="font-size: 22px"></i>
            ...
            <div>
                <h3><?=$this->lang->line('application_service_order_data');?></h3>
                            ...
                            <?php if ($response->$fieldname != null) :  ?>
                                ...
                                <?php else : ?>
                                    ...
                                    <td><?php foreach ($response->$fieldname as $field) : ?>
                                            <a href="<?=base_url()?>files/serviceorders/<?=$field?>" target="_blank"><i class="dripicons dripicons-document" style="font-size: 22px"></i>
